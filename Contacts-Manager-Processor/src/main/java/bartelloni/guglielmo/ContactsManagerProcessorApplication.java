...
public class ContactsManagerProcessorApplication {
    ...
    CommandLineRunner commandLineRunner(RabbitTemplate template){
        return args ->{
            Contact contact = Contact.builder().name("Nome").surname("Cognome").phone("3248989883").lat("-16.95832").lon("-64.06230").otherInfo("Altre info").address("via della madonna 13").build();
